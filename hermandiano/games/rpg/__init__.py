...
from hermandiano import exl, Datos, bot, bot_i

#Variables universales
obj_i = exl.sheet_by_name("objetos")
desc_i = exl.sheet_by_name("descripciones")
mons_i = exl.sheet_by_name("monstruos")
areas_i = exl.sheet_by_name("areas")

#Devuelve un strings con los huecos del inventario
def visualizainv( database, server : str, miembro : str):
...
def porvida( vida : int, maxvida : int):
    corazones = ""
...
def dmgrealizado( atq : int, defe : int, prec : int):
    dmg = int(0)
...
async def delreact(canal : int, usuario : int, mensaje : int, emoji=None, todo : bool=False):
    try:
        channel = bot.get_channel(canal)
        message = await channel.fetch_message(mensaje)
        user = bot.get_user(usuario)
        if todo:
...
def acenemigo( p, idmem, idserver):
    respuesta = [1, 2, False]
    idioma = p["jugadores"][idserver]["configs"]["idioma"]
    mons = p["jugadores"][idserver][idmem]["luchando"]
    player = p["jugadores"][idserver][idmem]
    dmg = dmgrealizado(mons["atq"], p["jugadores"][idserver][idmem]["def"], mons["prec"])
    player["hp"] -= dmg
    if dmg > 0:
        if player["hp"] <= 0:
            respuesta[2] = True
            player["hp"] = 0
            playerhp = porvida(player["hp"], player["maxhp"])
            mongrafichp = porvida(mons["hp"], mons["maxhp"])
            a2 = discord.Embed(
...
def permisoparajugar( canal : str, server : str):
...
def niveles_habla( database, miembro, server):
...
@bot.command(aliases=['comenzar', 'start', 'iniciar'])
async def empezar( ctx):
    server = str(ctx.message.guild.id)
    database = Datos.alldata()
    idioma = database["servers"][server]["configs"]["idioma"]
    if permisoparajugar(str(ctx.message.channel.id), server):
        miembro = str(ctx.author.id)
        database = niveles_habla(database, miembro, server)
        ...
        await error_02(idioma, ctx)

#Comando que muestra el inventario del usuario
@bot.command(aliases=['inv', 'objetos', 'objs', "inventory", "items"])
async def inventario( ctx):
    ...
    if permisoparajugar(str(ctx.message.channel.id), server):
        ...
        database = niveles_habla(database, miembro, server)
        if "juego" in database["servers"][server]["miembros"][miembro]:
            inv = visualizainv(database, server, miembro)
            inven = discord.Embed(
            ...
            await error_01(idioma, ctx)
    else:
        await error_02(idioma, ctx)

#Comando que muestra la estadística del usuario 
@bot.command(aliases=['estadisticas', 'estadísticas', 'est', 'stat'])
async def stats( ctx):
    server = str(ctx.message.guild.id)
    database = Datos.alldata()
    idioma = database["servers"][server]["configs"]["idioma"]
    if permisoparajugar(str(ctx.message.channel.id), server):
        ...
        if "juego" in database["servers"][server]["miembros"][miembro]:
            ...
            vida = porvida(juego["hp"], juego["maxhp"])
...
@bot.command(aliases=['explore', 'aventura', 'adventure'])
async def explorar( ctx):
    if permisoparajugar(str(ctx.message.channel.id), str(ctx.message.guild.id)):
        server = str(ctx.message.guild.id)
        database = Datos.alldata()
        miembro = str(ctx.author.id)
        idioma = database["servers"][server]["configs"]["idioma"]
                    ...
                    for msj in database["servers"][server]["mensajes"]:
                        if msj != "ddf":
                            if database["servers"][server]["mensajes"][msj]["miembro"] == miembro and database["servers"][server]["mensajes"][msj]["categoria"] == "e":
                                try:
                                    canal = bot.get_channel(int(database["servers"][server]["mensajes"][msj]["canal"]))
                                    if canal != None:
...
@bot.command(aliases=['sleep', 'rest'])
    ...
    if msj in database["servers"][server]["mensajes"]:
        ...
        if miembro != "730804779021762561" and miembro != "736966021528944720": #bot id
            ...
            if reaction.emoji.id == None:
                ...
                emoji = bot.get_emoji(reaction.emoji.id)
            if miembro != mensaje_d["miembro"]:
                await delreact(reaction.channel_id, reaction.user_id, reaction.message_id, emoji)
            else:
                z = False
                accion = None
                ...
                if z:
                    if not mensaje_d["procesando"]:
                        ...
                        if mensaje_d["categoria"] == "e":
                            ...
                            await delreact(reaction.channel_id, reaction.user_id, reaction.message_id, todo=True)
                            juego = database["servers"][server]["miembros"][miembro]
                            juego["juego"]["luchando"] = luchando
                            mensaje_d["categoria"] = "l"
                            mensaje_d["reacts"] = {"🔪":"atacar", "💨":"huir"}
                            mensaje_d["procesando"] = False
                            mongrafichp = porvida(luchando["hp"], luchando["maxhp"])
                            playerhp = porvida(juego["juego"]["hp"], juego["juego"]["maxhp"])
                            lucha = discord.Embed(
                            ...
                            mensaj = None
                            try:
                                canal = bot.get_channel(reaction.channel_id)
                                ...
                                try:
                                    ...
                                    if reaction.member.dm_channel != None:
                                        ...
                                        server_name = bot.get_guild(int(server)).name
                            ...
                            await delreact(mensaj.channel.id, mensaj.author.id, mensaj.id, emoji)
                            Datos.mensaje(server, msj, mensaje_d)
                            Datos.miembro(server, miembro, juego)
                        ...
                        """elif mensaje_d["categoria"] == "l":
                            for react in mensaje_d["reacts"]:
                                await delreact(reaction.channel_id, 736966021528944720, reaction.message_id, react)
                            await delreact(reaction.channel_id, reaction.user_id, reaction.message_id, emoji)
                            if accion == "atacar":
                                player = p["jugadores"][idserver][idmem]
                                prec = 95
                                ...
                                dmg = dmgrealizado(player["atq"], player["luchando"]["def"], prec)
                                if dmg > 0:
                                    ...
                                    if player["luchando"]["hp"] <= 0:
                                        ...
                                        playerhp = porvida(player["hp"], player["maxhp"])
                                        mongrafichp = porvida(player["luchando"]["hp"], player["luchando"]["maxhp"])
                                        lucha = discord.Embed(
                                        ...
                                        await mensaj.edit(embed=lucha)
                                        await sleep(2)
                                        enenm = acenemigo(p, idmem, idserver)
                                        p = enenm[0]
